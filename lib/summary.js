...
export function computeDashboardSummary(customers, nowMs = Date.now()) {
  ...
  for (const customer of customers) {
    ...
    // preserve original semantics: monthlyLoanGiven uses createdAt
    const createdTs = Number(new Date(customer.createdAt).getTime());
  ...
  for (const tx of normalizedTransactions) {
    ...
    // profit allocation for this payment using shared helper
  ...
  const recentTransactions = normalizedTransactions.slice(0, 5).map((t) => ({
